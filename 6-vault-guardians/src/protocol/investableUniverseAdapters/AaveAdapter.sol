...
// e it acts as bridge between vault logic and Aave's lending protocol (via IPool)
contract AaveAdapter {
     ...
     */
    function _aaveInvest(IERC20 asset, uint256 amount) public {
    ...
    function _aaveDivest(
        IERC20 token,
        uint256 amount
    ) public returns (uint256 amountOfAssetReturned) {
        i_aavePool.withdraw({
            asset: address(token),
            amount: amount,
            to: address(this)
