...
contract YeildERC20 is ERC20 {
    ...
    function _update(
        ...
        if (_to == owner) {
            ...
            uint256 userAmount = (_value * USER_AMOUNT) / PRECISION;
            uint256 ownerAmount = (_value * FEE) / PRECISION;
            count = 0;
            super._update(_from, _to, userAmount);
            super._update(_from, owner, ownerAmount);
        } else {
            count++;
            super._update(_from, _to, _value);
        }
